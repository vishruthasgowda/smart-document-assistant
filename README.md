# 🧠 DocuBrain AI — Smart Document Assistant

DocuBrain AI is a conversational document assistant. Upload a PDF, DOCX, or TXT file and ask questions about it in natural language — the app retrieves the most relevant chunks of your document and generates accurate, context-aware answers using a large language model.
...
- **Multi-format support** — Upload PDF, DOCX, or TXT files, including scanned PDFs (via automatic OCR fallback).
...
- **Robust text extraction** — Handles scanned PDFs (OCR), unusual `.txt` encodings, and tables inside `.docx` files.
...
| Text extraction | `pypdf`, `python-docx`, `pytesseract`, `pdf2image` |
...
2. Install dependencies:
   ...
   ```
3. For OCR support on scanned PDFs, also install Tesseract OCR and Poppler on your system (not just via pip):
   - **Windows:** Install [Tesseract OCR](https://github.com/UB-Mannheim/tesseract/wiki) and [Poppler for Windows](https://github.com/oschwartz10612/poppler-windows/releases), then add both to your system PATH.
   - **Deployed (Streamlit Cloud):** handled automatically via `packages.txt`.
